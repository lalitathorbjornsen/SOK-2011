#Innlevering 1.1
#Laste ned pakkene til 3.1 data
library(dplyr)
library(plyr)
library(ggplot2)
library(WDI)
...
# Lage datasettet du vil bruke til analysen din (dette viser eksempel 2019)
...
# 4 desp
#Du kan lage mer beskrivende navn til variablene og lage tabeller ved 
# bruk av ulike pakker. Her nede bruker jeg “vtable”.
...
df <- subset(df_growth2019, select = c("avg_gdpgrowth", "avg_p", "avg_nsy", "avg_educ", 
                                       "avg_gi", "avg_gx", "avg_n", "avg_nry", "gdppc0"))
# Gi beskrivende navn til variablene (i samme rekkefølge som de ligger i datasettet)
labs <- c("Årlig vekstrate i BNP pc 2000-2019 (%)", "Årlig befolkningsvekst (%)", 
          "Årlig netto-sparing (andel av BNP)", "Antall år i utdanning (15+)", "Vekstrate i investerineger",
          "Vekstrate i eksport", "Reduksjonsrate i naturressurser", 
          "Vekstrate i arbeidskraft", "BNP per innbygger i år 2000") 
...
# outliners (fjerne de landene som viser signifikant, her 2)
...
# outliners (fjerne de landene som viser signifikant, her 1)
...
# grafene til emoirisk analyse
suppressPackageStartupMessages(library(scales))

# BNP vs befolkningsvekst
...
plot1 <- ggplot(df_growth2019_n, aes(x = avg_p , y = ln_gdppc, na.rm = TRUE)) +
  ...
  geom_text(aes(label=iso3c),size=4, hjust = 0, nudge_x = 0.05) +
  ...
  geom_smooth(aes(col=region), method="lm", se=FALSE) +
  ...
  # scale_y_continuous(trans = 'log2', labels=scales::dollar_format(), breaks=c(500, 2000, 8000, 32000, 120000)) + # logaritmere BNP pc og velg hvilke "ticks" som skal vises
  scale_x_continuous(breaks=c(-1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5 )) # Velg vilke "ticks" som skal vises på x-akselen
plot1


# BNP vs sparing, må finne ut av x-verdiene, prosent her, log på y
plot2 <- ggplot(df_growth2019_n, aes(x = avg_nsy , y = ln_gdppc, na.rm = TRUE)) +
  ...
  geom_jitter(aes(col=region)) + 
  geom_smooth(aes(col=region), method="lm", se=FALSE) +
  scale_x_continuous(labels=scales::dollar_format())  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
  theme(legend.text = element_text(size = 10,color="black"))+  # Bestem font-størrelse på legend
  scale_colour_manual(values = rainbow(9)) +# Velg farger til bobblene
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
...
plot3 <- ggplot(df_growth2019_n, aes(x = avg_educ , y = ln_gdppc, na.rm = TRUE)) +
  ...
  geom_jitter(aes(col=region)) + 
  geom_smooth(aes(col=region), method="lm", se=FALSE) +
  scale_x_continuous()  + # Legg til et dollar tegn til y-akselen
  scale_size_area(guide = "none", max_size = 14) + #Ta vekk legend for befolkningsstørrelse
  theme(legend.text = element_text(size = 10,color="black"))+  # Bestem font-størrelse på legend
  scale_colour_manual(values = rainbow(9)) +# Velg farger til bobblene
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
...
# Årlig vekstrate i BNP per innbygger på den ene siden og sparing
...
# Årlig vekstrate i BNP per innbygger vs utdanning
plot5 <- ggplot(df_growth2019_n, aes(x = avg_educ , y = avg_gdpgrowth, na.rm = TRUE)) +
  xlab("År i utdanning") + # Beskrivelse for x-akselen
  ylab("Årlig vekstrate i BNP per innbygger") + # Beskrivelse for y-akselen
  geom_text(aes(label=iso3c),size=4, hjust = 0, nudge_x = 0.05) +
  theme_minimal(base_size = 14, base_family = "Georgia") + # Tekststørrelse og font
  geom_point(aes(size = poptot, color = region), alpha = 0.8) + # Størrelse (farge) på bobblene avhenger befolkningsstørrelse (region)
...
# fjerne landene(3) før man kjører modellen 
# regres
suppressPackageStartupMessages(library(sjPlot))
suppressPackageStartupMessages(library(sjmisc))
suppressPackageStartupMessages(library(sjlabelled))

model1 <- lm(avg_gdpgrowth  ~ avg_p + avg_nsy + avg_educ + avg_gi + avg_gx 
             + avg_nry + avg_n + ln_gdppc0, data = no_outliers1)

#Endre varibelnavn 
pl <- c(
  `(Intercept)` = "Intercept",
  "avg_p" = "Vekstrate i befolkningen",
  "avg_nsy" = "Sparing som andel av BNI (netto)", 
  "avg_educ" = "Antall år i skole (befolkning 15+)", 
  "avg_gi" = "Årlig vekstrate i investeringer",
  "avg_gx" = "Årlig vekstrate i eksport)",
  "avg_nry" = "Årlig reduksjonsrate i naturressurser",
  "avg_n" = "Vekst i arbeidskraft",
  "ln_gdppc0" = "log av BNP per innbygger år 2000"
  
)

regfin <- tab_model(model1,  
          p.style = "stars",
          pred.labels = pl
          )
regfin
